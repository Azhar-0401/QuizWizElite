...
const {User,Admin,Questions}=require('../db/index')
...
router.post('/courses', authfunction, async (req, res) => {
    try {
      const course = new Questions(req.body);
      await course.save();
...
router.put('/courses/:courseid',authfunction, async(req,res)=>{
    const course= await Questions.findByIdAndUpdate(req.params.courseid,req.body,{new: true});
    if(course){
...
router.get('/courses',authfunction, async (req,res)=>{
    const courses= await Questions.find({});
    res.json({courses});
})
router.get('/me/:courseId', authfunction, async (req, res) => {
    const course = await Questions.findById(req.params.courseId);
    if (course) {
      res.json(course);
    } else {
